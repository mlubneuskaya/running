...
def plot_kinematic_chain(
    df, annotations_df, side, part, axis, fps, show_annotations=True
):
    ...
    plt.plot(
        time_x, normalize(acc), label="Acceleration", color="#2ca02c", ls=":", lw=1.5
    )
    ...
    plt.title(
        f"{side.capitalize()} {part.capitalize()} Kinematic Profile",
        fontsize=14,
        pad=35,
    )
    ...
    plt.legend(
        by_label.values(), by_label.keys(), loc="upper left", bbox_to_anchor=(1.02, 1)
    )
