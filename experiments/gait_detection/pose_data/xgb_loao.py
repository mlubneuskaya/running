"""Leave-one-athlete-out cross-validation for XGBoost (pose data).

Uses best hyperparameters from stage3 XGBoost tuning.  For each held-out
athlete, trains XGBoost on all other athletes and saves per-frame softmax
probabilities for the held-out records.

Usage
-----
    python -m experiments.gait_detection.pose_data.xgb_loao \
        --config configs/experiments/pose_data/xgb_loao.yaml
...
def main(cfg: ExperimentConfig, best_params: dict, output_dir: str) -> None:
    ...
    with mlflow.start_run(run_name="pose_xgb_loao"):
        ...
        for fold_i, (train_records, val_records, athlete) in enumerate(loao_splits(records), 1):
            ...
            for rec in val_records:
                ...
                probs = clf.predict_proba(feats).astype(np.float32)
...
if __name__ == "__main__":
    ...
    output_dir      = raw.get("output_dir", "data/output/gait/pose_data/xgb_loao")
